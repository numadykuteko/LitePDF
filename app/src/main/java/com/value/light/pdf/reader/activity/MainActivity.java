...
import com.value.light.pdf.reader.component.ConfirmDialog;
import com.value.light.pdf.reader.component.PdfOptionDialog;
import com.value.light.pdf.reader.component.RenameFileDialog;
import com.value.light.pdf.reader.component.SettingSortDialog;
import com.value.light.pdf.reader.data.FileData;
import com.value.light.pdf.reader.utils.CommonUtils;
import com.value.light.pdf.reader.utils.DateTimeUtils;
import com.value.light.pdf.reader.utils.SnackBarUtils;
import com.value.light.pdf.reader.utils.ToastUtils;
import com.value.light.pdf.reader.utils.adapter.FileListAdapter;
import com.value.light.pdf.reader.utils.adapter.OnFileItemWithOptionClickListener;
import com.value.light.pdf.reader.utils.file.FileUtilAsyncTask;
import com.value.light.pdf.reader.utils.file.FileUtils;
...
public class MainActivity extends BaseActivity implements OnFileItemWithOptionClickListener, SettingSortDialog.SettingSortSubmit {
    ...
    protected void startDownloadFromGoogleDrive(Uri uri) {
        AsyncTask.execute(() -> {
            try {
                ...
                runOnUiThread(() -> {
                    checkFilePathGet(uri, file.getPath());
                });

            } catch (Exception e) {
                runOnUiThread(() -> {
                    checkFilePathGet(uri, null);
                });
            }
        });
    }
    ...
    private void getFileList() {
        mAsyncTask = new FileUtilAsyncTask(getApplication(), result -> {
            runOnUiThread(() -> updateData(result));
        }, mCurrentSortBy);
    ...
    private void showPermissionIssueArea() {
        mNoPermissionArea.setOnClickListener(v -> {
